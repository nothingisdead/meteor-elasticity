...
var publish = function(template) {
	return function(key) {
		...
		var promise = client.search.call(this, index_name, template.query(query_string), offset, limit, type);
		...
		promise.then(function(data) {
			...
			var cursor = Meteor[index.collection].find(query, index.options.cursor_options || {});
